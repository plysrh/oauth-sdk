...
import { setupOAuthMocks, setupOAuthErrorMocks, setupSlowOAuthMocks, setupGitHubOAuthRoute, setupGoogleOAuthRoute } from './oauth.mock';

test.describe('AuthFlow Demo', () => {
  ...
  test('should redirect to GitHub OAuth when clicking GitHub button', async ({ page }) => {
    ...
    await setupGitHubOAuthRoute(page);
  ...
  test('should redirect to Google OAuth when clicking Google button', async ({ page }) => {
    ...
    await setupGoogleOAuthRoute(page);
