import { AuthConfig, Provider, User, AuthProvider } from './types';
import { GitHubProvider } from './modules/github';
import { GoogleProvider } from './modules/google';

/**
 * Main AuthFlow class for handling multi-provider OAuth authentication
 * 
 * @example
 * ```typescript
 ...
 * 
 * // Get login URL
 * const loginUrl = authFlow.getLoginUrl('github');
 * 
 * // Handle OAuth callback
 * const user = await authFlow.handleCallback('github', code);
 * ```
 */
export class AuthFlow {
  ...
  /**
   * Initialize AuthFlow with OAuth provider configurations.
  ...
  /**
   * Handle OAuth callback by exchanging authorization code for user info.
  ...
  async handleCallback(provider: Provider, code: string): Promise<User> {
    ...
    return await providerInstance.getUserInfo(tokens.accessToken);
  }
